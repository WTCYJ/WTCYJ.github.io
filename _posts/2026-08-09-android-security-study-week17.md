...
포기한 것을 분명히 적어두겠습니다. **단일 변수 비교는 이 장비에서 못 합니다.** 그 대신 22주차의 소스 diff 분석 비중을 키우는 것이 현실적인 선택입니다.
...
한계를 적어둡니다. **Cuttlefish 를 실제로 띄우지는 않았습니다.** KVM 가용은 확인했지만 디스크가 빠듯해 시도하지 않았습니다. 하네스의 두 이미지는 동일 조건이 아니고, 이것이 가장 큰 제약입니다. 같은 API 레벨의 과거 리비전 이미지를 받을 수 있다면 교란 변수를 줄일 수 있는데 아직 확인하지 못했습니다. 프로브가 다루는 항목도 여덟 개뿐이라, 18주차에 CVE 를 고른 뒤 그에 맞는 프로브를 따로 써야 합니다.
