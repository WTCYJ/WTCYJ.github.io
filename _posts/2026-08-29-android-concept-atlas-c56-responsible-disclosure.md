...
C56은 정책·절차 모듈이라 CVD 자체는 AVD로 측정할 대상이 아니다. 다만 이 세션이 실제로 수행한 검증 작업이 이 모듈의 핵심 주장을 직접 뒷받침한다.

**1) 이 글의 검증 방법론 자체가 C56이 말하는 "좋은 리포트"의 형태다(질문 5).** 검증 블록에 기록된 실행과 관측은 이렇다.
...
서명된 최소 증거 앱을 빌드·설치하고, 도구 버전과 원시 출력을 보존해 동일 명령을 다시 돌릴 수 있게 만든 것 — 이게 질문 5가 "이긴다"고 한 리포트의 형태다. `api33-baseline.md`·`host-verification.md`가 곧 "ASan 트레이스 첨부, 버전 X–Y"에 해당하는 재현 근거다. "가끔 크래시" 대신 재현 가능한 최소 증거를 남긴다는 원칙을, 이 문서가 스스로 지키고 있다.

**2) 링크·이미지 감사가 통과해 증거 참조가 실제로 해석된다.** Jekyll 빌드와 링크·이미지 감사가 함께 완료됐고, 상단 검증 화면(evidence-environment.png)이 렌더된다. 증거 앱·기준 로그·호스트 검증 로그로 가는 참조가 깨지지 않고 이어진다는 뜻 — 리포트의 "재현 가능"이 링크 수준에서 확인된 것이다.
