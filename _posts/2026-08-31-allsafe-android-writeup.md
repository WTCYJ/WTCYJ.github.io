...
excerpt: "요즘 안드로이드 앱 보안을 손으로 익히고 싶어서 일부러 취약하게 만든 앱 Allsafe를 골라 처음부터 끝까지 직접 뜯어봤습니다. 로그 유출·하드코딩 자격증명부터 exported 컴포넌트, ContentProvider SQL 인젝션, WebView 로컬 파일 읽기, 네이티브 라이브러리 역산, DexClassLoader 임의코드 실행, 직렬화 객체 위변조까지 — 에뮬레이터에서 하나하나 재현하며 화면과 로그로 확인한 기록입니다. 잘 된 것만이 아니라 처음에 막힌 지점, 요즘 안드로이드에선 아예 안 되던 것까지 있는 그대로 적었습니다."
---
...
> **범위**: 제 소유 에뮬레이터 위의 학습용 타깃. 실제 서비스가 아닌 훈련 앱 분석입니다.

요즘 안드로이드 앱 보안을 눈이 아니라 손으로 익히고 싶었습니다. 그래서 일부러 취약하게 만든
연습용 앱을 찾다가 Allsafe를 골랐습니다. 이 앱이 마음에 든 건, 흔한 CTF처럼 "플래그를 가져와라"가
아니라 **OkHttp·Firebase·RootBeer·JNI 네이티브 같은 현업 라이브러리로 실제 앱처럼 취약점을 심어
뒀다**는 점이었습니다. 덕분에 "문제를 푼다"기보다 "실제 앱을 감사한다"는 감각으로 접근할 수
있었습니다.

저는 루팅한 실기기 대신 x86_64 에뮬레이터를 세팅해 진행했습니다. 이 글은 제가 이 앱을 처음부터
끝까지 직접 뜯어보며 겪은 과정을 정리한 기록입니다. **잘 풀린 것만 적지 않았습니다.** 처음에 헛다리
짚고 막혔던 지점, 그리고 "코드에는 취약점이 분명히 있는데 요즘 안드로이드에선 아예 실행조차 안 되는"
경우까지 있는 그대로 남겼습니다. 저는 이 마지막 부분 — **"코드에 있다"와 "지금 이 기기에서 실제로
터진다"는 다르다** — 가 이 글에서 제일 하고 싶은 이야기입니다.

문제를 번호순으로 하나씩 풀 수도 있었지만, 다 풀고 나서 다시 보니 성격이 비슷한 것끼리 묶는 편이
훨씬 눈에 잘 들어왔습니다. 그래서 실행은 순서대로 했지만, 정리는 취약점의 성격별로 묶었습니다.

---

## 0. 먼저 부딪힌 벽 (Secure Flag Bypass)

사실 저는 첫 번째 문제를 풀기도 전에 벽에 부딪혔습니다. 화면을 캡처해서 블로그에 올리려고
`screencap`을 찍었더니 **까만 화면(23KB짜리 빈 PNG)** 만 나오더군요. 원인을 찾아보니 런처 액티비티가
창을 만들 때 스크린샷·화면녹화를 막는 `FLAG_SECURE`를 걸고 있었습니다.
...
이걸 먼저 풀지 않으면 이후 어떤 화면도 증거로 남길 수가 없었습니다. 마침 이게 뒤에 나올 챌린지 중
하나라서, "이왕 이렇게 된 거 이 문제부터 같이 풀자"는 생각으로 손을 댔습니다. 처음엔 이미 떠 있는
창에 `clearFlags`를 호출해봤는데 소용이 없었습니다 — 이미 secure로 태어난 표면은 되돌릴 수 없기
때문이었습니다. 그래서 방향을 바꿔 **앱이 뜨기 전(spawn 시점)에 `setFlags` 자체를 후킹**해서
`FLAG_SECURE`(0x2000) 비트를 벗겨 냈습니다.
...
앱은 여러 창 옵션을 정수 하나에 비트별로 모아 두기 때문에, 통째로 0으로 만들면 다른 기능까지 꺼져
...
훅이 걸리자마자 같은 `screencap`이 **까만 23KB에서 정상 103KB로** 바뀌었습니다. 이 한 번의 후킹이
챌린지 "Secure Flag Bypass"의 정답이면서, 동시에 이후 모든 화면 캡처의 전제 조건이 되어 줬습니다.
아래 홈 화면이 그 첫 증거입니다.

![Frida 로 FLAG_SECURE 를 벗긴 뒤 정상적으로 캡처된 Allsafe 홈 화면 — 이전에는 같은 명령이 검은 화면만 반환했습니다](/assets/img/allsafe-android/allsafe-home.png)

개인적으로 이 문제가 좋았던 건, `FLAG_SECURE`의 성격을 첫 화면부터 체감하게 해줬기 때문입니다.
이건 어깨너머 훔쳐보기나 악성 접근성 서비스의 화면 캡처를 늦추는 UX 보호일 뿐, 디바이스를 통제하는
공격자(루팅·Frida) 앞에서는 종이 한 장입니다. 민감 화면을 가리는 용도로는 의미가 있어도, 보안 경계로
믿으면 안 된다는 걸 몸으로 배웠습니다.
...
첫 묶음은 "앱이 스스로 비밀을 흘리는" 부류입니다. 공격이랄 것도 없이, 앱이 남겨 둔 걸 주워 담기만
하면 됩니다.
...
필드에 값을 입력하고 키보드의 완료(IME_ACTION_DONE)를 누른 뒤 logcat을 봤습니다.
...
제가 방금 친 값이 평문 그대로 로그에 찍혔습니다. `READ_LOGS`를 가진 앱이나 ADB 접근이 가능한
상황이면 사용자가 입력한 값을 그대로 수집할 수 있습니다. 쉬운 문제지만, 릴리스 빌드에서 로그를
지우지 않아 인증 응답이나 토큰이 새는 사고는 실제로도 흔합니다.
...
이 모듈은 안내문이 스스로 "이 프래그먼트에 하드코딩된 username:password 조합이 2개 있다"고
알려 줍니다. 저는 소스를 보는 대신, 이왕이면 실제로 하듯 **설치된 APK를 리버싱**해서 찾기로 했습니다.
base.apk를 당겨 dex와 리소스를 뒤졌습니다.
...
`https://admin:password123@dev.infosecadventures.com`이었습니다. 두 번째는 개발 서버 URL 안에
계정을 통째로 박아 둔 형태라 특히 눈에 띄었습니다. 클라이언트에 심은 비밀은 난독화를 해도 결국
문자열 테이블이나 dex 상수 풀에 남습니다 — "숨긴다"가 아니라 "아예 클라이언트에 두지 않는다"가
정답인 이유를 다시 확인했습니다.
...
이 앱은 디버그 가능(`android:debuggable="true"`)하게 빌드돼 있어서, `run-as`로 앱의 내부 저장소를
그대로 읽을 수 있었습니다. 루팅 기기나 백업에서도 똑같이 열립니다.
...
(Keystore 기반)로 보호해야 합니다. 평문 prefs는 디바이스에 접근할 수 있는 누구에게나 열린 파일입니다.
...
먼저 **MD5** 해시 버튼으로 `password`를 해싱해 봤습니다. 무솔트 평문 MD5라 레인보우 테이블로 즉시
역산됩니다.
...
**AES**는 더 심각합니다. 키가 코드에 박혀 있으니 누구나 복호할 수 있고, ECB 모드는 같은 평문 블록이
같은 암호 블록으로 나와 패턴이 드러납니다. 말로만 하면 와닿지 않아서, 박힌 키로 직접 왕복을 돌려
봤습니다.
...
키만 알면 암호문이 그대로 평문으로 돌아오고, 같은 블록이 같은 암호로 반복되는 것까지 눈으로
확인했습니다. 키는 서버나 Keystore에 두고, 모드는 최소 CBC/GCM이어야 하며, 비밀번호 저장에는
MD5가 아니라 bcrypt·scrypt·Argon2 같은 느린 해시를 써야 합니다.
...
이 묶음은 풀면서 제일 재밌었던 부류입니다. 공통점은 "판정을 클라이언트가 한다"는 것인데, 판정 코드와
데이터가 전부 기기 안에 있으니 정적으로 값을 캐거나 동적으로 반환을 뒤집으면 그대로 뚫립니다.
...
`4863`을 입력하니 바로 통과했습니다. 4자리 PIN을 클라이언트에서 검사하는 건 무차별 대입에도, 이런
상수 추출에도 무력합니다.
...
그런데 여기서 예상 밖의 일이 있었습니다 — **제 깨끗한 에뮬레이터는 RootBeer가 루팅으로 판정하지
않았습니다.** 아무 조작도 안 했는데 "not detected"가 떴습니다. 이유를 파고들어 보니, 빌드 태그가
`test-keys`가 아니라 `dev-keys`이고, `/system/xbin/su`는 존재하지만 SELinux 때문에 앱
프로세스(untrusted_app)에서는 `stat`조차 막혀 보이지 않기 때문이었습니다.

그래서 챌린지의 의도(Frida 연습)를 제대로 보이기 위해, `isRooted`의 반환을 **양방향으로** 조종해
봤습니다.
...
`true`로 강제하면 로그에 `RootBeer.isRooted() -> forced TRUE`가 찍히면서 아래처럼 "루팅됨"으로
바뀌고, `false`로 강제하면 다시 "미탐지"로 돌아옵니다. 판정 자체를 후킹으로 통째로 통제할 수 있다는
걸 두 화면으로 남겼습니다. 루팅 탐지가 앱 프로세스 안에서 이뤄지는 한, 그 안에 들어온 계측 도구
앞에서는 참·거짓이 공격자의 손에 있다는 걸 직접 보고 나니 확실히 이해됐습니다.
...
비밀번호 검증을 네이티브(`libnative_library.so`)로 옮겨 놓은 문제입니다. "네이티브로 옮기면 안전하다"는
흔한 오해를 겨냥한 건데, 정작 검증 로직은 단순 XOR이었습니다.
...
릴리스 .so는 스트립돼 있어 평문 "supersecret" 문자열은 남지 않았습니다(위의 `p`는 미사용이라 컴파일러가
제거). 하지만 목표 암호문과 키가 남아 있으니, 손으로 XOR만 되돌리면 됐습니다.
...
앱에 `supersecret`를 넣으니 통과했고, Frida로 네이티브 메서드 호출을 계측해 보니 인자와 반환이 그대로
보였습니다.
...
로직이 단순하면 아키텍처를 네이티브로 바꿔도 정적 역산이나 동적 후킹으로 그대로 뚫린다는 걸
확인했습니다.
...
달라서 기존 앱을 지우고 패치본을 새로 깔아야 했는데, 다시 실행해 버튼을 누르니 흐름이 바뀐 게 바로
보였습니다.
...
디컴파일→한 줄 수정→재서명이 이렇게 쉽다는 걸 손으로 해보니, 라이선스 체크·기능 게이트·안티치트를
클라이언트 분기 하나에 의존하면 안 되는 이유가 확 와닿았습니다.
...
호출할 수 있습니다. Allsafe는 리시버·서비스·프로바이더·프록시 액티비티를 죄다 열어 뒀길래, 이 묶음은
대부분 UI를 거치지 않고 `adb`로 바로 찔러 봤습니다.

### 3-1. Insecure Broadcast Receiver

`NoteReceiver`가 exported이고, 받은 extra로 URL을 조립해 요청을 보냅니다. 여기서 두 가지가
문제였습니다 — **서버 주소를 호출자가 준다**는 점, 그리고 **관리자 토큰이 하드코딩**돼 있다는 점입니다.
...
권한 하나 없는 제 명령이 Allsafe를 시켜 **하드코딩된 관리자 토큰을 제가 지정한 호스트로 전송**하게
만들었습니다(SSRF + 비밀 유출). exported 리시버는 서명 권한으로 보호하거나 내부 전용으로 닫아야
한다는 걸 실제 URL로 확인했습니다.

### 3-2. Deep Link Exploitation

`DeepLinkTask`가 `allsafe://infosecadventures/congrats` 스킴을 처리하고, 쿼리 `key`를 `R.string.key`와
...
딥링크로 도달하는 화면이 인증·상태 검증 없이 열리면, 브라우저 링크 한 줄로 내부 기능이 트리거됩니다
(딥링크 CSRF).
...
가진 호스트 앱을 대리인으로 삼아 **사용자 동의 없는 도청**이 됩니다. 마이크처럼 민감한 동작을 하는
서비스는 절대 exported여선 안 됩니다.
...
selection/projection을 신뢰하지 말고 화이트리스트·파라미터 바인딩으로 다뤄야 합니다.
...
대리인(confused deputy)입니다. 넘겨받은 Intent는 컴포넌트/액션을 화이트리스트로 검증한 뒤에만
전달해야 합니다.
...
username에 `'or'1'='1'--`를 넣으면 비밀번호 조건이 주석 처리되면서 조건이 항상 참이 되어 사용자
...
덤프된 해시 `21232f29...`는 "admin"의 MD5입니다(앞의 Weak Crypto와 연결됩니다). 앱 내부 SQLite라도
웹과 똑같이 파라미터 바인딩(`?`)을 써야 한다는 걸 다시 확인했습니다.

![SQL Injection — 'or'1'='1'-- 로 인증을 우회하고 사용자·해시를 덤프한 Toast](/assets/img/allsafe-android/c09-sql-injection.png)

### 4-2. Vulnerable WebView

WebView가 자바스크립트와 파일 접근을 모두 켜 뒀습니다.
...
입력이 유효 URL이 아니면 `loadData`, 유효 URL이면 `loadUrl`로 처리하는데, 두 경로 모두 위험했습니다.
첫째, `<script>alert(1337)</script>`를 넣으면 WebView 안에서 임의 자바스크립트가 실행됩니다. 둘째,
`file:///etc/hosts`를 넣으면 **기기의 로컬 파일이 WebView에 그대로 렌더**됩니다. (사실 처음 alert을
시도했을 땐 안 떠서 loadData가 막힌 줄 알았는데, 화면 레이아웃이 밀려서 실행 버튼을 잘못 누른
것뿐이었습니다. 좌표를 다시 맞추니 바로 떴습니다.)
...
파일 접근을 켠 WebView는 `file://`로 앱 샌드박스·시스템 파일을 열 수 있습니다. 파일 접근은
끄고(`setAllowFileAccess(false)`), 신뢰 못 할 입력을 WebView에 절대 로드하지 말아야 합니다.
...
있어서(`https://allsafe-8cef0.firebaseio.com`), 규칙이 공개(public read)라면 앱을 거치지 않고 REST로
바로 읽힙니다. 그래서 앱이 읽는 `secret` 대신 **루트 경로**를 찔러 봤습니다.
...
이게 이 글에서 개인적으로 제일 재밌었던 발견입니다. 앱은 `secret`만 보여 주는데, **루트를 읽으니 앱에는
아예 노출되지 않은 `flag` 값까지 통째로** 나왔습니다. 클라이언트가 특정 노드만 읽는다고 해서 DB가 그
노드만 준다는 뜻이 아닙니다 — 접근 제어는 Firebase 규칙에서 해야 하고, 규칙이 열려 있으면 전체가
열린 것입니다.
...
버킷이 취약해서가 아니라, **구글이 2024년 9월 무료(Spark) 요금제의 Storage 접근을 회수**해서 백엔드
자체가 402를 반환합니다. 코드와 설정(gs URL·api_key)으로 취약 구조는 확인되지만, 정책 변경으로 실제
다운로드는 불가능했습니다. 되는 척하지 않고 있는 그대로 남깁니다.

---

## 6. 코드 로딩

마지막 전, 가장 파고들 맛이 났던 묶음입니다. 남의 코드나 데이터를 검증 없이 믿을 때 무슨 일이
일어나는지 보여 줍니다.
...
저는 처음에 (B) 벡터부터 시도했습니다. `/sdcard/Download/`에 dex를 심으면 되겠거니 했는데,
**계속 아무 일도 안 일어났습니다.** 파고들어 보니 이 앱의 `targetSdk`가 35라 scoped storage가
강제되고, `requestLegacyExternalStorage`는 무시됩니다. 그래서 앱이 자기가 만들지 않은
`/sdcard/Download/allsafe_updater.apk`를 읽으려 하면 `Permission denied`로 막혀 로딩 자체가 안
일어났습니다. **이 벡터는 요즘 안드로이드에서 사실상 죽어 있었습니다(API 29 이하에서나 유효).**
"코드에는 있는데 지금 기기에선 안 되는" 딱 그 경우였습니다.

그래서 (A) 벡터로 방향을 틀었습니다. 이쪽은 저장소와 무관하게 살아 있었습니다. 패키지명이
`infosecadventures.allsafe.`로 시작하는 앱을 하나 만들고, 그 안에 `infosecadventures.allsafe.plugin.Loader.loadPlugin()`을
심으면, Allsafe가 시작될 때 **그 코드를 자기 프로세스로 끌어와 실행**합니다. 오버시큐어드가 정리한
"서드파티 패키지 컨텍스트를 통한 임의코드 실행" 기법입니다.

PoC 앱(`infosecadventures.allsafe.poc`)의 `loadPlugin()`이 Allsafe의 Application 컨텍스트를 리플렉션으로
얻어, **호스트의 프라이빗 내부 저장소**에 파일을 쓰게 했습니다 — 그 UID로만 가능한 동작입니다.
...
제 별도 앱의 코드가 **Allsafe의 UID(10181)·프로세스·프라이빗 저장소**에서 실행됐습니다. 다른 앱의
컨텍스트를 `CONTEXT_INCLUDE_CODE | CONTEXT_IGNORE_SECURITY`로 만들어 그 코드를 부르는 건,
공격자에게 자기 프로세스를 통째로 내주는 것과 같습니다. 신뢰할 수 없는 출처의 dex/context는 절대
로드해선 안 됩니다. 참고로 이 모듈 안내문도 "코드 실행 방법이 두 가지"라고 알려 주는데, 실제로 하나는
현대 안드로이드에서 죽고 하나는 살아 있는 걸 직접 갈라 본 게 이 문제의 백미였습니다.
...
외부 저장소의 직렬화 파일은 손댈 수 있습니다. `ROLE_AUTHOR`와 `ROLE_EDITOR`가 둘 다 11바이트라,
직렬화 스트림의 길이 프리픽스를 건드리지 않고 **제자리 바이트 패치**만으로 권한을 올릴 수 있겠다
싶었습니다. 그런데 여기서 한참 헤맸습니다 — `adb push`로 파일을 덮으니 소유자가 바뀌어 앱이 못
읽더군요(EACCES, scoped storage/FUSE). 소유권을 복구하려고 chown도 해봤지만 FUSE라 소용없었습니다.
결국 **파일을 재생성하지 않고, root `dd`로 오프셋 153의 11바이트만 제자리에서 덮어써서** 소유권을
보존하는 방법으로 풀었습니다.
...
역직렬화 입력을 검증 없이 신뢰하면 권한 상승은 물론, (신뢰할 수 없는 클래스라면) 코드 실행까지
이어질 수 있습니다. 민감 상태는 외부 저장소에 두지 말고, 역직렬화 시 무결성 검증과 화이트리스트가
필요합니다. FUSE 소유권 때문에 삽질한 덕에, 파일을 "덮어쓰기"와 "제자리 수정"이 시스템 관점에서
얼마나 다른지도 덤으로 배웠습니다.
...
마지막은 정직하게 선을 그어야 하는 문제였습니다. Certificate Pinning 모듈은 OkHttp
`CertificatePinner`로 `httpbin.io`를 피닝합니다. 코드를 읽어 보니 구현이 **자가 치유**형이라
흥미로웠습니다 — 먼저 일부러 틀린 핀으로 요청해 예외 메시지에서 실제 서버 핀 해시를 파싱한 뒤, 그
해시로 다시 피닝합니다. 그래서 앱은 정상 상황에서 항상 연결에 성공합니다.
...
Frida로 `CertificatePinner.check` / `check$okhttp`를 무력화하는 훅을 넣어 두긴 했지만, 여기서 솔직하게
적겠습니다. **피닝 우회의 진짜 목적은 가로채기 프록시(Burp/mitmproxy)의 CA를 앱이 받아들이게
만드는 것**입니다. 프록시를 세우지 않은 이 환경에서는 "핀 검증을 건너뛰게 만들었다"까지는 계측으로
보였지만, 클리어텍스트 트래픽을 실제로 가로채 보여 주는 단계는 프록시 + CA 설치가 있어야 합니다.
게다가 릴리스 R8 빌드라 내부 핀검증 호출 경로가 훅 시그니처와 어긋나는 부분도 있었습니다. 그래서 이
모듈만큼은 "분석 + 기법 + 연결 성공 확인"까지로 정직하게 남기고, 트래픽 가로채기는 프록시 셋업을
전제로 다음에 따로 검증할 지점으로 표시해 둡니다.
...
전부 풀고 나서 제 손에 남은 교훈은 하나로 모였습니다. **기기 안에 있는 것은 무엇도 비밀이 아니고,
밖에서 들어오는 것은 무엇도 신뢰할 수 없습니다.** 로그·prefs·리소스·dex·네이티브 문자열은 전부
추출됐고, 클라이언트 판정(PIN·루트·네이티브·방화벽)은 정적 추출이나 동적 후킹으로 뒤집혔으며,
exported 컴포넌트와 검증 없는 입력(SQL·WebView·직렬화·패키지 컨텍스트)은 곧장 공격 표면이 됐습니다.

그런데 이 앱을 풀면서 개인적으로 제일 많이 남은 건, "되는 것"보다 **"안 되는 것"의 경계**였습니다.
scoped storage는 `/sdcard` 기반 dex 로딩 벡터를 실제로 죽였고, 요금제 정책 변경은 Firebase Storage
백엔드를 회수했으며, RootBeer는 제 깨끗한 AVD를 루팅으로 판정하지 않았습니다. **취약점은 코드에
그대로 있어도, 플랫폼과 환경이 도달성을 바꿉니다.** 그래서 저는 앞으로도 "코드에 있다"와 "지금 이
기기에서 실제로 터진다"를 구분해서 적으려고 합니다 — 이 앱이 제게 준 가장 큰 습관입니다.

방어 측 정리도 같은 축입니다. 비밀은 서버·Keystore로, 판정은 서버로, 컴포넌트는 기본 닫힘으로, 입력은
파라미터 바인딩·화이트리스트로, 그리고 신뢰할 수 없는 코드·데이터·컨텍스트는 로드하지 않기. 하나하나는
교과서 같은 이야기지만, 이 앱은 그 교과서를 어겼을 때 실제로 무슨 일이 벌어지는지를 화면과 로그로
보여 줬습니다. 다음엔 인증서 피닝을 프록시까지 세워서 끝까지 가 보고 싶습니다.
