...
이 앱은 첫 문제를 건드리기도 전에 저를 한 번 붙잡았습니다. 진행 화면을 캡처해서 블로그에 쓰려고
`screencap`을 찍었는데, 결과물이 새까만 빈 PNG(23KB)로만 나오더군요. 원인을 뒤져 보니 런처 액티비티가
창을 띄우면서 스크린샷과 화면녹화를 막는 `FLAG_SECURE`를 걸어 두고 있었습니다.
...
이걸 안 풀면 뒤에 나올 문제들 화면을 하나도 못 남기는데, 마침 이것도 챌린지 중 하나라 겸사겸사 먼저
손댔습니다. 처음엔 이미 떠 있는 창에다 `clearFlags`를 불러 봤는데 전혀 먹히지 않았습니다. 한 번
secure로 태어난 창은 나중에 풀어도 되돌려지지 않더군요. 그래서 창이 만들어지기 전, 그러니까 앱이
spawn되는 순간에 `setFlags` 자체를 가로채서 `FLAG_SECURE`(0x2000) 비트만 떼어내기로 했습니다.
...
`flags & ~FLAG_SECURE`는 캡처 방지 비트 딱 하나만 끄고 다른 창 설정은 손대지 않는 연산입니다.
안드로이드는 창 옵션 여러 개를 정수 한 값에 비트로 몰아 담기 때문에, 그냥 0으로 밀어 버리면 멀쩡한
옵션까지 같이 꺼집니다. 그래서 문제되는 비트만 콕 집어 지웠습니다.
...
훅을 걸어 두고 `screencap`을 다시 찍으니, 새까맣던 23KB가 멀쩡한 103KB로 바뀌었습니다. 이 후킹 한
방이 "Secure Flag Bypass" 문제의 답이자, 뒤에 나오는 모든 화면 캡처의 발판이 됐습니다. 아래가 그렇게
얻은 첫 화면입니다.

![Frida 로 FLAG_SECURE 를 벗긴 뒤 정상적으로 캡처된 Allsafe 홈 화면 — 이전에는 같은 명령이 검은 화면만 반환했습니다](/assets/img/allsafe-android/allsafe-home.png)

개인적으로 이 문제가 좋았던 이유는, `FLAG_SECURE`가 어디까지 막아 주는 방어인지 시작부터 알게
해줘서입니다. 이건 옆에서 훔쳐보거나 악성 접근성 서비스가 화면을 긁어 가는 걸 늦추는 UX용 장치일 뿐,
기기 자체를 쥔 공격자(루팅·Frida) 앞에서는 힘을 못 씁니다. 민감한 화면을 가리는 데는 쓸 만해도 이걸
진짜 보안 경계로 믿으면 안 된다는 걸 첫 문제부터 확인한 셈입니다.
