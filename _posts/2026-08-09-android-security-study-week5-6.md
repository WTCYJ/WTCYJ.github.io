...
이 글은 24주 Android 보안 학습 로드맵의 5~6주차 기록입니다. 3~4주차에 직접 만들어둔 테스트 앱을 이번에는 개발자가 아니라 분석가의 자리에서 다시 열어봤습니다. 이 글에서는 정적 분석이 무엇인지, apktool과 jadx가 각각 어떤 일을 하는지를 먼저 정리한 뒤, 자작 리포트 스크립트가 무엇을 곧바로 잡아냈고 직접 탐지의 경계 너머 셋을 어떤 대체 경로로 마저 표면화했는지를 실제 산출물과 함께 살펴보겠습니다.
...
Android 앱의 배포 단위인 **APK**는 사실상 zip 아카이브입니다. 다만 그 안의 내용물이 사람이 바로 읽을 수 있는 형태가 아닙니다. `AndroidManifest.xml`은 텍스트 XML이 아니라 바이너리로 인코딩돼 있고, 개발자가 작성한 Java·Kotlin 코드는 컴파일과 변환을 거쳐 `classes.dex`라는 [**DEX**(Dalvik Executable)](https://source.android.com/docs/core/runtime/dex-format) 바이트코드로 들어가 있습니다. 그래서 정적 분석의 첫 단계는 언제나 "읽을 수 있는 형태로 되돌리기"입니다. 여기서 도구가 둘로 갈립니다.
...
용어 두 가지를 더 풀어두겠습니다. [**exported 컴포넌트**](https://developer.android.com/guide/topics/manifest/activity-element#exported)란 다른 앱이 인텐트로 직접 호출할 수 있도록 공개된 컴포넌트를 말합니다. Android 12(API 31)부터는 인텐트 필터가 달린 컴포넌트라면 `android:exported`를 명시하지 않으면 설치 자체가 거부되도록 바뀌었는데([동작 변경 문서](https://developer.android.com/about/versions/12/behavior-changes-12#exported)), 이 앱은 그 값을 일부러 `true`로 열어 분석 대상으로 삼았습니다. [**딥링크**](https://developer.android.com/training/app-links/deep-linking)는 `memovault://notice`처럼 특정 URI를 앱의 특정 화면으로 연결해주는 진입 경로이고, 이 경로를 열어두면 외부 앱이나 웹 페이지가 그 화면을 띄울 수 있게 됩니다.
...
여기서 grep의 경계가 드러납니다. grep은 한 줄씩 패턴을 대조하는 도구라서 "여기에 `new File(...)`이 있다"까지만 말할 수 있습니다. 그 인자가 상수인지, 외부에서 들어온 값이 변수 몇 개를 건너 도착한 것인지는 한 줄만 봐서는 알 수 없습니다. 오염 흐름은 여러 줄, 여러 함수, 때로는 여러 파일에 걸쳐 있어서 — 한 함수 안에 갇힌 intra-procedural 흐름을 넘어 함수 경계를 건너는 inter-procedural 흐름이라서 — **줄 단위 검색만으로는 이어붙일 수 없는 영역입니다.** 이번 구간의 성과 하나는 직접 탐지가 닿는 경계가 정확히 이 지점이라는 것을 답을 아는 앱으로 확정한 것이고, 그래서 그 경계 너머를 겨냥한 대체 경로(4-3의 입력·싱크 대조표)를 처음부터 설계에 넣었습니다.
...
궁금했던 것은 "취약점을 찾을 수 있나"가 아니라 **"자동 분석이 어디까지 곧바로 닿고, 그 경계 너머는 어떤 방법으로 메우나"**였습니다. 답을 아는 상태에서 도구를 돌려보는 경험은 이번이 아니면 하기 어렵습니다.

---

## 2. 자동 분석 파이프라인 구성과 실행

`static-analyze.sh <apk> <label>` 한 줄로 디코드 → 디컴파일 → 리포트까지 돕니다. 49초 걸렸고 jadx가 소스 2,972개를 뽑았습니다. 파이프라인은 apktool로 매니페스트·리소스를 복원하고, jadx로 DEX를 Java 유사 소스로 역컴파일한 뒤, 두 산출물을 각각의 강점대로 훑어 표를 뽑습니다. 선언 정보(exported·권한·딥링크)는 apktool 산출물에서, 코드 로직(WebView 설정·호출 지점)은 jadx 산출물에서 가져옵니다.

나온 표는 아홉 개입니다. 컴포넌트 노출, 권한, 앱 전역 플래그, 딥링크, 저장소 호출 지점, 네트워크 호출 지점, 로그 출력 지점, WebView 설정, 하드코딩 의심 문자열.

정적 분석은 이렇게 "실행하지 않은 산출물"을 다루지만, 그 산출물이 기기 위에서 어떤 형태가 되는지 알아두면 표를 읽는 근거가 단단해집니다. 이 글의 대상 앱은 아니지만 같은 플랫폼(에뮬레이터, API 33)에서 임의의 앱을 설치해 실측해 보면, 우리가 zip으로 열어본 그 APK는 기기에서 `/data/app/~~<무작위>/<패키지명>-<무작위>/base.apk`(`splits=[base]`)에 자리잡습니다. 디렉터리에 붙는 무작위 문자열은 경로 열거를 어렵게 만드는 장치입니다. 그리고 APK 안에서 우리가 jadx로 읽은 그 `classes.dex`는 설치 시점에 `oat/x86_64/base.odex`와 `base.vdex`로 변환되는데, 실측한 dexopt 상태는 `status=verify`(reason=install)였습니다. 설치 순간에는 전면 AOT 컴파일이 아니라 **검증(verify)만** 하고 실제 기계어 컴파일은 뒤늦은 백그라운드 dexopt로 미룬다는 뜻입니다. 부트 클래스패스는 별도로 `/system/framework/oat/x86_64/services.art`처럼 미리 컴파일된 이미지로 올라와 있습니다. 정적 분석이 읽는 DEX와 런타임이 실제로 실행하는 odex/vdex가 이렇게 갈린다는 점은 7~8주차 동적 분석에서 다시 쓰입니다.
...
여기서 [`addJavascriptInterface`](https://developer.android.com/reference/android/webkit/WebView)는 앱의 Java 객체를 WebView 안의 자바스크립트에 그대로 노출시키는 API입니다. 이 객체에 붙은 메서드는 페이지의 자바스크립트가 `window.<등록이름>.<메서드>()` 형태로 직접 호출할 수 있습니다. Android 4.2(API 17)부터는 `@JavascriptInterface`가 붙은 public 메서드만 노출되도록 조여졌는데, 그 이전에는 노출 객체의 모든 public 메서드가 열려서 자바스크립트가 `getClass()`로 리플렉션 사슬을 타고 `Runtime.exec`까지 도달하는 원격 코드 실행([CVE-2012-6636](https://nvd.nist.gov/vuln/detail/CVE-2012-6636))이 성립했습니다. 이 앱은 API 17 이후 규칙을 지켜 애너테이션이 붙은 메서드만 열지만, 문제는 그 메서드가 곧 세션 토큰과 API 키를 돌려주도록 설계돼 있다는 것입니다. 브릿지에 붙은 메서드 이름은 `getSessionToken`, `getUsername`, `getApiKey`, `readFile`입니다. 딥링크로 임의 URL을 띄울 수 있으니 외부 페이지가 이 메서드들을 부를 수 있다는 뜻입니다.
...
여기서 "앱 권한"이 무엇을 뜻하는지는 샌드박스 경계로 설명됩니다. 이 글의 대상 앱은 아니지만 같은 플랫폼에서 임의의 앱을 실측하면, 각 앱은 고유한 리눅스 UID(예: `userId=10176`, 셸 표기 `u0_a176`)를 받고 자기 데이터 디렉터리 `/data/data/<패키지>`는 `drwx------`(0700, 소유자만)로 잠깁니다. `readFile`이 앱 권한으로 도는 이상, 이 브릿지는 바로 그 0700 사택 안쪽 — 다른 앱은 발도 못 들이는 영역 — 을 외부 웹페이지에게 읽어주는 창구가 됩니다. WebView 렌더러는 실측상 `webview_zygote`(이것 자체가 `zygote64`의 자식 프로세스)에서 포크된 별도 프로세스에서 돌지만, 브릿지 객체는 앱 프로세스 쪽에 앱 UID로 살아 있으므로 렌더러 격리가 이 노출을 막아주지는 못합니다.

### 3-3. 정답표 대조 - 직접 탐지가 7개를 곧바로 잡다

심어둔 10개 중 자동 리포트가 곧바로 잡은 것은 7개였습니다. 아래는 정답표와 리포트를 나란히 놓고 대조한 결과입니다. 마지막 열에 `→ 4-3`이 붙은 셋은 "못 찾은 것"이 아니라 grep 직접 탐지의 경계 밖에 있는 흐름형이고, 4-3에서 입력·싱크 대조표로 마저 표면화합니다.
...
| 하드코딩 키 | ○ | 하드코딩 문자열 |
| 클라이언트 측 인가 | 흐름형 | 입력·싱크 대조표 → 4-3 |
| 서버 응답 파일명 사용 | 흐름형 | 싱크 포착 → 4-3 |
| intent extra 임의 파일 읽기 | 흐름형 | 입력·싱크 대조표 → 4-3 |

---

## 4. 결과 해석 - 데이터 흐름을 대체 경로로 메운 근거

### 4-1. grep이 원리적으로 볼 수 없는 것

직접 탐지의 경계 밖에 있던 셋은 공통점이 분명했습니다. **전부 데이터 흐름 문제입니다.**
...
이것이 도구를 신뢰할 때 가장 조심할 지점이라고 봅니다. 표가 깔끔하게 나오면 "다 봤다"는 느낌이 드는데, 정작 심각한 셋은 이 표가 아니라 4-3의 대조표에서 드러납니다. 그래서 경계를 먼저 인정하고 대조표를 따로 붙이는 설계가 필요했습니다.
...
표가 결론을 내주지는 않지만 **어디를 읽어야 하는지는 알려줍니다.** 이것을 붙이고 나서 심어둔 10개 전부가 — 7개는 직접 탐지로, 3개는 이 대조표로 — 표면화됐습니다. 도구 하나로 다 잡지 못하는 영역을 도구를 하나 더 설계해서 온전히 덮은 셈입니다.
...
이번 구간에서 측정을 잘못 읽을 뻔한 지점이 네 곳 있었고, 네 곳 모두 원인까지 짚어 바로잡았습니다. 아래 기록은 실패담이 아니라 "왜 그렇게 보였는지"를 끝까지 확인해 리포트의 신뢰도를 끌어올린 과정입니다. 결과 표 한 줄보다 이 네 번의 교정이 이번 구간에서 더 오래 남을 것 같습니다.
...
다만 성격이 조금 달라졌습니다. 지난번에는 도구 출력을 잘못 읽은 것이었고, 이번에는 **grep 직접 탐지가 닿는 경계가 어디인지**를 답을 아는 앱으로 정확히 확정한 쪽입니다. 데이터 흐름 셋은 그 경계 너머에 있어 줄 단위 검색만으로는 나오지 않지만, 경계를 알고 나니 그 너머를 겨냥한 입력·싱크 대조표로 셋 다 표면화할 수 있었습니다.

답을 아는 앱으로 도구를 돌려본 것이 이번 구간에서 제일 값어치 있었습니다. 덕분에 직접 탐지가 7개에서 멈추는 경계를 눈으로 보고, 그 너머 3개까지 대체 경로로 마저 짚어 심어둔 10개를 온전히 덮었습니다. 모르는 앱이었다면 7개에서 만족하고 지나쳤을 자리를, 이번엔 끝까지 채운 셈입니다.

---

## 참고 자료

- [DEX(Dalvik Executable) 포맷 명세 — Android Open Source Project](https://source.android.com/docs/core/runtime/dex-format)
- [`<activity android:exported>` — Android Developers](https://developer.android.com/guide/topics/manifest/activity-element#exported) · [Android 12 동작 변경: exported 명시 의무화](https://developer.android.com/about/versions/12/behavior-changes-12#exported)
- [딥링크 만들기 — Android Developers](https://developer.android.com/training/app-links/deep-linking)
- [`WebView` 클래스 레퍼런스(addJavascriptInterface·@JavascriptInterface) — Android Developers](https://developer.android.com/reference/android/webkit/WebView) · [CVE-2012-6636 — NVD](https://nvd.nist.gov/vuln/detail/CVE-2012-6636)
- [OWASP Mobile Application Security Testing Guide(MASTG)](https://mas.owasp.org/MASTG/) · [MASVS](https://mas.owasp.org/MASVS/)
- [`kotlin.Metadata` 애너테이션 — Kotlin](https://kotlinlang.org/api/core/kotlin-stdlib/kotlin/-metadata/)
