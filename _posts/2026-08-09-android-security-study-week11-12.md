...
**[MASVS](https://mas.owasp.org/MASVS/)**(Mobile Application Security Verification Standard)는 OWASP가 정리한 모바일 앱 보안 검증 표준으로, 확인해야 할 항목을 통제 그룹으로 묶어둔 목록입니다. MASVS-STORAGE(저장), MASVS-NETWORK(통신), MASVS-AUTH(인증), MASVS-PLATFORM(플랫폼 상호작용), MASVS-CODE(코드 품질), MASVS-RESILIENCE(변조 저항) 같은 그룹으로 나뉩니다. **[MASTG](https://mas.owasp.org/MASTG/)**는 그 각 항목을 실제로 어떻게 시험하는지 적어둔 테스트 가이드입니다. 이번 진단의 핵심 소재인 딥링크·WebView·ContentProvider는 모두 [MASVS-PLATFORM](https://mas.owasp.org/MASVS/09-MASVS-PLATFORM/) 그룹에 속합니다.

진단에서 이 기준이 필요한 이유는 빠뜨림을 줄이기 위해서입니다. 눈에 띄는 것부터 파고들면 흥미로운 하나에 시간을 다 쓰고 저장소·통신 항목은 빈칸으로 남기 쉽습니다. MASVS 체크리스트는 각 통제 그룹을 강제로 한 번씩 훑게 만들어, 그 빈칸을 남기지 않고 표를 채우게 해줍니다.
...
이게 통하는 이유를 한 단계 더 파보면 이렇습니다. WebView는 `file://` 스킴을 로드할 수 있고, `WebView`의 파일 접근은 오래된 targetSdk에서 기본 허용입니다. 여기에 앱이 `data`/`content` 스킴만 걸러내고 `file` 스킴을 걸러내지 않았기 때문에, 검증 없는 `/web` 경로가 `file:///data/data/…/Prefs.xml` 을 그대로 렌더한 것입니다. 만약 앱이 [`setAllowUniversalAccessFromFileURLs(true)`](https://developer.android.com/reference/android/webkit/WebSettings#setAllowUniversalAccessFromFileURLs(boolean)) 까지 켜뒀다면, `file://` 오리진의 스크립트가 다른 오리진에도 접근할 수 있어 렌더에서 그치지 않고 읽은 값을 외부로 보내는 구조가 됩니다. 이 앱은 그 설정까지는 켜져 있지 않았지만, 그 한 줄 차이로 심각도가 어디까지 갈 수 있는지는 짚어둘 만합니다.

플랫폼이 WebView를 어떻게 격리하는지도 별도 에뮬레이터(API 33)에서 눈으로 확인해뒀습니다. WebView의 렌더러는 `zygote64` 가 아니라 전용 `webview_zygote` 에서 갈라져 나온 별도 프로세스로 뜹니다.
...
이 격리는 렌더러가 시스템을 건드리지 못하게 막아주는 방어입니다. 다만 그것은 시스템을 보호하는 것이고, **앱이 스스로 자기 데이터 디렉터리의 `file://` 을 로드하겠다고 결정하는 것까지 막아주지는 않습니다.** 플랫폼 방어는 멀쩡히 동작하는데, 취약점은 앱이 검증 없이 `loadUrl` 을 호출한 그 지점에 있습니다.
...
`<permission>` 요소에서 [`protectionLevel`](https://developer.android.com/guide/topics/manifest/permission-element) 을 생략하면 기본값이 `normal` 입니다. Android의 [권한 보호 수준](https://developer.android.com/guide/topics/permissions/overview#permission-groups)은 `normal`(설치 시 자동 부여) · `dangerous`(런타임에 사용자 동의) · `signature`(같은 서명 키로 서명된 앱에만 부여)로 나뉘는데, 같은 개발자만 접근하도록 데이터를 지키려면 이 Provider는 `signature` 여야 합니다. 즉 이건 "권한을 안 걸었다"가 아니라 **"권한을 걸었는데 가장 약한 수준이라 아무 앱이나 통과한다"** 는 쪽이라, 매니페스트를 눈으로만 보면 정반대로 읽히기 쉽습니다.
...
`endsWith` 는 URL을 **문자열**로 보고 끝부분만 비교합니다. `#` 뒤의 프래그먼트나 경로·쿼리에 허용 도메인 문자열을 얹으면 실제 접속지는 딴 데인데 검사만 통과합니다. 제대로 하려면 문자열이 아니라 **호스트를 파싱해서 비교**해야 합니다. Android는 [`android.net.Uri`](https://developer.android.com/reference/android/net/Uri#getHost()) 의 `getHost()` 로 호스트만 뽑아 정확히 일치(또는 신뢰하는 서브도메인)만 허용하는 방식을 씁니다. `String.endsWith` 로 도메인을 검사하는 코드는 딥링크·오픈 리다이렉트·WebView 화이트리스트에서 반복적으로 우회되는 대표적인 안티패턴입니다. 관련 지침은 [App Links·딥링크 처리](https://developer.android.com/training/app-links)에 정리돼 있습니다.
...
이 "샌드박스가 정상 동작한다"는 전제는 말로만 두지 않고 실제 값으로 확인해뒀습니다. 이 앱의 `dumpsys` 출력에 `uid=10176` 이 찍혀 있는데(3-3), 별도 에뮬레이터(API 33)에서 같은 대역의 앱을 실측하니 데이터 디렉터리는 이렇게 잠겨 있었습니다.

```
$ adb shell ls -la /data/data/<pkg>
drwx------ 4 u0_a176 u0_a176 4096 /data/data/<pkg>
```

디렉터리 모드는 `drwx------`(0700)이고 소유자는 앱 전용 UID `u0_a176`(=userId 10176, 같은 `dumpsys` 에 찍힌 값)입니다. 커널의 [애플리케이션 샌드박스](https://source.android.com/docs/security/app-sandbox)가 UID 단위로 격리하기 때문에, 다른 앱은 이 디렉터리를 열 수 없습니다. 그래서 평문 저장은 단독으로는 보통에 머무릅니다. 격리 자체는 멀쩡하니까요.
...
## 5. 세 번의 자기교정 - 관측을 신뢰할 수 있게 만든 과정

이번 구간에서 결과의 신뢰를 세운 건 취약점을 찾은 것 자체가 아니라, 측정이 틀릴 수 있는 세 지점을 각각 잡아내 바로잡은 과정이었습니다. 진단은 도구가 뱉은 값을 그대로 옮기는 작업이 아니라, 그 값이 진짜인지 한 겹 더 확인하는 작업입니다. 아래 셋은 그 확인이 실제로 일한 자리입니다.

### 재현과 정적 근거를 분리해 보고서의 무게를 세웠다

보고서 초안에서 12건을 같은 형식으로 나열했습니다. 초안을 다시 읽는 검토 단계에서 한 가지를 잡아냈습니다. 딥링크로 자격증명을 화면에 띄운 것은 제가 직접 실행해 스크린샷까지 있는데, 외부 패키지 코드 로드(`createPackageContext` 로 다른 앱의 클래스를 실행하는 경로)는 **코드로 경로를 확정한 것**이었습니다. 기기 재현까지 가려면 공격 측 앱을 따로 만들어야 하는데, 그건 13~14주차 이후의 범위라 이번에는 정적 근거로 경로를 확정하는 선에서 멈췄습니다.

둘을 같은 톤으로 적으면 읽는 사람은 전부 기기에서 확인된 것으로 받아들입니다. 그래서 각 항목에 **"기기 확인" / "정적 근거"** 를 붙이고, 마지막에 재현 요약 표를 따로 뒀습니다. 12건 중 8건이 기기 확인, 4건이 정적 근거입니다.

이렇게 적고 나니 보고서가 약해 보일까 걱정했는데, 오히려 반대라고 생각을 바꿨습니다. 확인 범위를 명시한 8건이, 전부 확인한 척한 12건보다 신뢰가 갑니다. **범위를 정확히 적는 것이 결과를 깎는 게 아니라 나머지의 무게를 올립니다.**
...
`KEYCODE_TAB` 으로 포커스를 옮기도록 바꿔 해결했습니다. 그리고 `input text` 는 `%s` 를 공백으로 해석하기 때문에 비밀번호 앞에 공백이 하나 붙는 것도 한 번 더 걸러내, 자동화가 정확히 자격증명을 넣도록 다듬었습니다.

사소한 문제지만 짚어둘 만한 지점이 있습니다. **화면을 확인하지 않고 탭 좌표만 믿으면 조용히 틀립니다.** 처음엔 "자격증명이 틀렸나" 부터 의심했는데 화면을 보니 입력 자체가 다른 칸에 들어간 것이었습니다 — 스크린샷 한 장이 원인을 정확히 가리켰습니다. 매 단계 화면으로 결과를 확인하는 습관이 7~8주차의 거짓 양성과 같은 뿌리를 막아줍니다.

### 새 앱에 맞춰 도구 필터를 조율했다

하드코딩 문자열 표가 DataBinding 생성 코드로 가득 찼습니다. 제 앱을 분석할 때는 없던 잡음입니다. 그 앱이 DataBinding을 안 썼기 때문입니다. 제외 규칙을 한 줄 더해 이 잡음을 걷어내고, 하드코딩 의심 문자열을 19행으로 추려 실제 비밀만 남겼습니다.

**도구는 지금까지 본 앱에 맞춰 조율돼 있습니다.** 처음 보는 앱에 대면 그 앱이 쓰는 빌드 도구·라이브러리에 맞춰 필터를 새로 손보게 됩니다. 진단 시간의 일부는 이 조율에 쓰인다고 잡아두는 편이 맞고, 조율을 마친 표는 그만큼 신호 대 잡음비가 좋아집니다.
...
범위도 명시적으로 적어둡니다. 이번 진단은 앱 바이너리와 로컬 에뮬레이터(API 33)로 대상을 못 박고, 그 안에서 8건을 기기로 확증하고 4건을 정적 근거로 확정했습니다. 나머지 확장은 다음 구간으로 이어지도록 배치했습니다. 공격 측 앱을 만들어야 재현되는 4건과 네이티브·난독화는 13~14주차의 주제이고, 다른 API 레벨 거동은 targetSdk 29 앱을 여러 기기에 올려 비교하는 후속 실험으로 남겨뒀습니다. 각 관측은 스냅샷·빌드 지문·로그로 재현성을 고정해뒀으므로, 범위를 좁게 잡은 만큼 그 안의 결과는 그대로 다시 밟을 수 있습니다. 5~6주차에 확인한 대로 자동 분석은 데이터 흐름을 놓치기 때문에, 이 12건은 "이 범위에서 확인된 것"이라고 정직하게 한정합니다.

다음은 13~14주차 네이티브·런타임 기초입니다. JNI와 ELF, 난독화가 분석에 어떤 영향을 주는지 다룹니다. 이번에 다음으로 넘긴 항목들이 그 구간과 그대로 이어집니다.

---

## 참고 자료

이번 구간에서 판단의 근거로 삼은 1차 출처입니다.

- OWASP [MASVS](https://mas.owasp.org/MASVS/) · [MASTG](https://mas.owasp.org/MASTG/) — 진단 기준과 테스트 절차
- [MASVS-PLATFORM](https://mas.owasp.org/MASVS/09-MASVS-PLATFORM/) — 딥링크·WebView·IPC(ContentProvider) 통제 그룹
- Android Developers, [`<permission>` 매니페스트 요소](https://developer.android.com/guide/topics/manifest/permission-element)와 [권한 개요](https://developer.android.com/guide/topics/permissions/overview) — `protectionLevel` 기본값과 `normal`/`signature` 차이
- Android Developers, [`WebSettings.setAllowUniversalAccessFromFileURLs`](https://developer.android.com/reference/android/webkit/WebSettings#setAllowUniversalAccessFromFileURLs(boolean)) · [`Uri.getHost()`](https://developer.android.com/reference/android/net/Uri#getHost()) — WebView 파일 접근과 호스트 파싱 검증
- Android Developers, [App Links·딥링크 처리](https://developer.android.com/training/app-links) — 딥링크 검증 권장 방식
- Android Open Source Project, [애플리케이션 샌드박스](https://source.android.com/docs/security/app-sandbox) — UID 단위 격리 모델
...
가장 크게 남은 것은 보고서 쓰는 방식이었습니다. 취약점을 찾는 것보다 **찾은 것과 찾았다고 생각한 것을 구분해 적는 일**이 어려웠습니다. 12건을 나란히 놓으면 그럴듯해 보이는데, 하나씩 "이건 내가 눌러봤나"를 물으니 넷은 정적 근거 쪽으로 재분류됐습니다. 그 넷을 지우지 않고 표시만 바꿔 남긴 것이 이번 보고서에서 제일 잘한 선택이라고 생각합니다.
