---
layout: post
title: "Android Security Concept Atlas Tier 3 | 학습 로드맵 — IPC·프레임워크"
date: 2026-08-29 23:18:00 +0900
category: 안드로이드
author: WTCY
...
이 글은 개별 모듈을 묶는 **학습 로드맵**이다. Tier 3의 공통 기준 실습은 전용 API 33 AVD에서 다음 항목으로 확인했다: `ls -l /dev/{binder,hwbinder,vndbinder}`, `service list`. binderfs의 세 Binder 노드와 255개 서비스 등록을 확인했다. 벤더 전용 HAL 트랜잭션이나 취약한 서비스 호출은 범용 AVD에 없으므로 공개 인터페이스·소스 분석으로 제한한다.

![Tier 3 가상 실습 기준 화면](/assets/img/android-concept-atlas/verified-api33/evidence-binder.png)
...
| C18 | [Parcel 직렬화·read/write 불일치](/posts/android-concept-atlas-c18-parcel-serialization-contract/) | 🟢 | ✅ | 쓰기/읽기 순서 불일치가 타입 혼동·메모리 손상으로 |
...
| C22 | [caller UID/PID·identity clearing·confused deputy](/posts/android-concept-atlas-c22-binder-caller-identity-confused-deputy/) | 🟢 | ✅ | `clearCallingIdentity` 오용이 confused-deputy로 |
...
> **완성 보고서**: C18과 C22는 Parcel 계약 및 Binder 호출자 identity를 증거 앱의 대조군으로 검증했습니다.
